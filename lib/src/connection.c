...
int connection_recv(const connection_t* connection, LPTSTR* buff) {
	SSIZE_T len;
	if ((*buff = (TCHAR*)malloc(sizeof(TCHAR) * (MSG_LEN + 1))) == NULL) {
		return -1;
	}
	memset(*buff, 0, sizeof(TCHAR) * (MSG_LEN + 1));
	len = recv(connection->socket, (char*)*buff, MSG_LEN, 0);
	if (len == -1 || len > sizeof(TCHAR) * MSG_LEN) {
		free(*buff);
		buff = NULL;
		return -1;
	}
	/*	Resize if string is shorter than len	*/
	if (realloc(*buff, sizeof(TCHAR) * (_tcslen(*buff) + 1)) == NULL) {
		free(*buff);
		buff = NULL;
		return -1;
	}
...
int connection_send(const connection_t* connection, LPCTSTR buff) {
	SSIZE_T len;
	if ((len = send(connection->socket, (char*)buff, sizeof(TCHAR) * _tcslen(buff), 0)) == -1) {
		return -1;
	}
	return len;
}
